...
internal static partial class Program
{
    ...
    private static void BooleanVectorAlgebraMatchesIndependentOracle()
    {
        ...
        for (var length = 0; length <= 8; length++)
        {
            ...
            {
                var vector = BooleanVectorFromMask(length, bits);
                vectors[bits] = vector;
                vectorCases++;
                if (!BooleanVectorMatchesMask(vector, length, bits))
                {
                    Fail($"raw value mismatch at length {length}, bits {bits}");
                }

                if (vector.Parity() != ((BooleanVectorPopulation(bits) & 1) != 0))
                {
                ...
                for (var distance = 0; distance <= length + 1; distance++)
                {
                    ...
                    if (!BooleanVectorMatchesMask(
                            vector.ShiftTowardHigherOrdinals(distance),
                            length,
                            higher) ||
                        !BooleanVectorMatchesMask(
            ...
            for (var left = 0; left < valueCount; left++)
            {
                var a = vectors[left];
                var notA = a.Not();
                if (!BooleanVectorMatchesMask(notA, length, (~left) & logicalMask) ||
                    !a.Or(a).Equals(a) ||
                    !a.And(a).Equals(a) ||
                    !a.Xor(a).IsEmpty ||
                ...
                for (var right = 0; right < valueCount; right++)
                {
                    orderedPairs++;
                    var b = vectors[right];
                    if (!BooleanVectorMatchesMask(a.Or(b), length, left | right) ||
                        !BooleanVectorMatchesMask(a.And(b), length, left & right) ||
                        !BooleanVectorMatchesMask(a.Xor(b), length, left ^ right) ||
                        !BooleanVectorMatchesMask(a.AndNot(b), length, left & ~right & logicalMask) ||
    ...
    private static void BooleanPrefixParityMatchesIndependentOracle()
    {
        ...
        for (var length = 0; length <= 10; length++)
        {
            ...
            for (var bits = 0; bits < valueCount; bits++)
            {
                var input = BooleanVectorFromMask(length, bits);
                ...
                {
                    scanCases++;
                    var expected = BooleanPrefixParityOracle(length, bits, carryIn, out var expectedCarry);
                    var result = input.PrefixParity(carryIn);
                    if (!BooleanVectorMatchesMask(result.Vector, length, expected) ||
                    ...
                    {
                        splitCases++;
                        var left = SliceBooleanVector(input, 0, split);
                        var right = SliceBooleanVector(input, split, length);
                        var leftResult = left.PrefixParity(carryIn);
                        var rightResult = right.PrefixParity(leftResult.CarryOut);
                        var joined = ConcatenateBooleanVectors(leftResult.Vector, rightResult.Vector);
        ...
        for (var length = 0; length <= 8; length++)
        {
            ...
            for (var bits = 0; bits < valueCount; bits++)
            {
                var scan = BooleanVectorFromMask(length, bits).PrefixParity(false);
            ...
            for (var left = 0; left < valueCount; left++)
            {
                ...
                {
                    linearCases++;
                    var xorScan = BooleanVectorFromMask(length, left ^ right).PrefixParity(false);
    ...
    private static void BooleanVectorLongAndTailCasesMatchOracle()
    {
        ...
        {
            var leftBits = CreateRandomBits(random, length);
            var rightBits = CreateRandomBits(random, length);
            var left = BooleanVectorFromBits(leftBits);
            var right = BooleanVectorFromBits(rightBits);

            if (!BooleanVectorMatchesBits(left, leftBits) ||
                !BooleanVectorMatchesBits(right, rightBits) ||
                !BooleanVectorMatchesBits(left.Or(right), CombineBooleanBits(leftBits, rightBits, static (a, b) => a || b)) ||
                !BooleanVectorMatchesBits(left.And(right), CombineBooleanBits(leftBits, rightBits, static (a, b) => a && b)) ||
                !BooleanVectorMatchesBits(left.Xor(right), CombineBooleanBits(leftBits, rightBits, static (a, b) => a != b)) ||
                !BooleanVectorMatchesBits(left.AndNot(right), CombineBooleanBits(leftBits, rightBits, static (a, b) => a && !b)) ||
                !BooleanVectorMatchesBits(left.Not(), leftBits.Select(static bit => !bit).ToArray()))
            {
                agrees = false;
            ...
            foreach (var distance in distances.Distinct())
            {
                ...
                if (!BooleanVectorMatchesBits(left.ShiftTowardHigherOrdinals(distance), expectedHigher) ||
                    !BooleanVectorMatchesBits(left.ShiftTowardLowerOrdinals(distance), expectedLower))
                {
                    agrees = false;
            ...
            while (cursor < length)
            {
                ...
                var chunk = SliceBooleanVector(left, cursor, cursor + chunkLength);
                var chunkScan = chunk.PrefixParity(chunkCarry);
                chunkStates = ConcatenateBooleanVectors(chunkStates, chunkScan.Vector);
                chunkCarry = chunkScan.CarryOut;
                cursor += chunkLength;
            }

            if (!BooleanVectorMatchesBits(scan.Vector, expectedStates) ||
                scan.CarryOut != state ||
    ...
    private static BooleanVector BooleanVectorFromMask(int length, int bits) =>
        BooleanVector.Create(length, Enumerable.Range(0, length).Where(ordinal => (bits & (1 << ordinal)) != 0));

    private static bool BooleanVectorMatchesMask(BooleanVector vector, int length, int bits)
    {
        if (vector.Length != length || vector.Population != BooleanVectorPopulation(bits))
        {
            return false;
        }
    ...
    private static int BooleanPrefixParityOracle(int length, int bits, bool carryIn, out bool carryOut)
    {
        var states = 0;
        var state = carryIn;
        for (var ordinal = 0; ordinal < length; ordinal++)
    ...
    private static BooleanVector SliceBooleanVector(BooleanVector source, int start, int end)
    {
    ...
    private static BooleanVector ConcatenateBooleanVectors(BooleanVector left, BooleanVector right)
    {
    ...
    private static int BooleanVectorPopulation(int value)
    {
        var count = 0;
        while (value != 0)
        {
    ...
    private static bool[] CreateRandomBits(Random random, int length)
    {
    ...
    private static BooleanVector BooleanVectorFromBits(IReadOnlyList<bool> bits)
    {
        var selected = new List<int>();
    ...
    private static bool BooleanVectorMatchesBits(BooleanVector vector, IReadOnlyList<bool> bits)
    {
    ...
    private static bool[] CombineBooleanBits(
